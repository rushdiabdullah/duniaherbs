...
export default async function BersalinPage() {
  ...
  const pickedIds = (content.bersalin_product_ids || '').split(',').map((s) => s.trim()).filter(Boolean);
  const byId = new Map(products.map((p) => [p.id, p]));
  const pickedProducts = pickedIds.map((id) => byId.get(id)).filter(Boolean) as typeof products;

  const fallbackProducts = products.filter(
    (p) => p.heat === 'Mild' || p.name?.toLowerCase().includes('limau') || p.name?.toLowerCase().includes('lime'),
  );
  const displayProducts = pickedProducts.length > 0 ? pickedProducts : (fallbackProducts.length > 0 ? fallbackProducts : products.slice(0, 3));
                    ...
                    <div className="rounded-2xl border border-amber-800/15 bg-gradient-to-br from-stone-900/80 to-stone-950/60 overflow-hidden transition hover:border-amber-600/30 hover:shadow-lg hover:shadow-amber-950/20">
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex items-center justify-between mt-3">
                          ...
                          {product.heat && <span className="text-[10px] px-2 py-0.5 rounded-full bg-amber-900/20 text-amber-400/70 border border-amber-800/20">{product.heat}</span>}
                        </div>
                      </div>
                    </div>
                  </Link>
